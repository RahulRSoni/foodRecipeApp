...
const LINKS = [
	{
		title: 'Company',
		items: {
			name: ['About us', 'Careers', 'Press', 'News'],
			path: ['/about', '#', '#', '#'],
		},
	},
	{
		title: 'Resource',
		items: {
			name: ['Blog', 'Newsletter', 'Events', 'Help center'],
			path: ['/blog', '#', '#', '/contact'],
		},
	},
];
...
export function Footer() {
	return (
		<footer className='relative w-full border-t-4 font-normal mt-16 transition-colors px-4 hover:text-blue-gray-900 bg-white overflow-y-auto'>
			<div className='mx-auto w-full max-w-7xl px-8 mt-3'>
						...
						{LINKS.map(({ title, items }) => (
							<ul key={title}>
								<Typography
									variant='small'
									color='blue-gray'
									className='mb-3  opacity-60 text-lg font-semibold order-last '>
									{title}
								</Typography>
								{items.name.map((link, index) => (
									<li key={link}>
										<Typography
											as='a'
											href={items.path[index]}
											color='gray'
											className='py-1.5 font-normal transition-colors hover:text-blue-gray-900 col-span-3'>
											{link}
										</Typography>
									</li>
								))}
				...
				<div className='mt-12 flex w-full flex-col items-center justify-center border-t border-blue-gray-400 py-4 md:flex-row md:justify-between'>
					<Typography
						...
						&copy; {currentYear}{' '}
						<a href='https://material-tailwind.com/'>dad's kitchen</a>. All
						Rights Reserved.
					</Typography>
